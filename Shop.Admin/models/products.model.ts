  ...
  try {
    ...
    if (formData.commentsToRemove) {
      if (typeof formData.commentsToRemove === 'string') {
        await axios.delete(`${API_HOST}/comments/${formData.commentsToRemove}`)
      } else {
        formData.commentsToRemove.forEach(async (commentId) => {
          await axios.delete(`${API_HOST}/comments/${commentId}`)
        })
      }
    }
    ...
    if (formData.imagesToRemove) {
      if (typeof formData.imagesToRemove === 'string') {
        await axios.post(`${API_HOST}/products/remove-image`, [
        ...
        await axios.post(`${API_HOST}/products/remove-image`, arrayToDel)
      }
    }
